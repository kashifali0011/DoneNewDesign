package com.dtech.done.doneapp.utilities.utils
...
object DateUtil {
    ...
    fun convertSingleDateTimePicker(startDate: String?): String {
        return if (startDate != null) {
            ...
            val splitedStartDate = startDate.split("T".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            val subSplitedStartDate = splitedStartDate[0].split("-".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray() //    val time1 = convertIn12HourFormatWithMinute(startDate)
